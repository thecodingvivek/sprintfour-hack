  ...
  return (
    <div className="mx-auto w-full max-w-4xl">
      <div className="flex gap-6 items-start">
        {result && (
          <nav className="w-28 shrink-0 sticky top-6 self-start hidden md:block">
            <div className="space-y-1">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className="w-full rounded-md px-3 py-1.5 text-left text-xs font-medium text-[#787774] hover:bg-[#F1F0ED] hover:text-[#2F3437] transition-colors"
                >
                  {item.label}
                </button>
              ))}
      ...
      <AnimatePresence>
        {policy && (
          ...
          >
            <div className="rounded-lg border border-[#EAEAEA] bg-white p-4 flex flex-col h-full space-y-3">
